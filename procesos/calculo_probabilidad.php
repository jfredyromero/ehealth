...
$sql = "SELECT * FROM datos_medidos WHERE fecha >= '$fecha_ini' and fecha <= '$fecha_fin'";
$result = $mysqli->query($sql);
$row = $result->fetch_array(MYSQLI_NUM);
if ($row == null) {
    $confir = 0;
    header('Location: /ehealth/interfaces/estadisticas/probabilidad.php?confir=' . $confir . '&fe_ini=' . $fecha_ini . '&fe_fin=' . $fecha_fin);
} else {
    $confir = 1;
